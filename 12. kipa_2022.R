#.패키지 설치 및 구동
library(dplyr)
library(ggplot2)

install.packages("foreign")
library(foreign)

#.데이터 불러오기
mental <- read.spss("KIPA_DATA_2022.SAV")
class(mental)#.객체유형 확인: list
mental <- as.data.frame(mental)#.데이터프레임으로 변환
class(mental)#.객체유형 확인: data.frame

mental2 <- read.spss("KIPA_DATA_2021.SAV")
class(mental2)#.객체유형 확인: list
mental2 <- as.data.frame(mental2)#.데이터프레임으로 변환
class(mental2)#.객체유형 확인: data.frame

#.변수 추출 후 이름 변경
mental <-mental %>%
  select(q32_2,q1_4,q32_1,q34_1,q55,d17,d1,d2,ara)%>%
  rename(suicide=q32_2,
         satisfaction=q1_4,
         loneliness=q32_1,
         family_belief=q34_1,
         wealth=q55,
         health=d17,
         sex=d1,
         age=d2,
         area=ara)

mental2 <-mental2 %>%
  select(q32_2,q1_4,q32_1,q34_1,q54,d17,d1,d2,ara)%>% #q54임
  rename(suicide=q32_2,
         satisfaction=q1_4,
         loneliness=q32_1,
         family_belief=q34_1,
         wealth=q54,
         health=d17,
         sex=d1,
         age=d2,
         area=ara)

#.변수 유형 및 빈도 확인
str(mental)

table(mental$suicide)
table(mental$health)
table(mental$wealth)
table(mental$satisfaction)

#.6개 변수간 관계분석을 위한 유형 변경
mental$suicide <-as.integer(mental$suicide)
mental$satisfaction <-as.integer(mental$satisfaction)
mental$loneliness <-as.integer(mental$loneliness)
mental$family_belief <-as.integer(mental$family_belief)
mental$wealth <-as.integer(mental$wealth)
mental$health <-as.integer(mental$health)

mental2$suicide <-as.integer(mental2$suicide)
mental2$satisfaction <-as.integer(mental2$satisfaction)
mental2$loneliness <-as.integer(mental2$loneliness)
mental2$family_belief <-as.integer(mental2$family_belief)
mental2$wealth <-as.integer(mental2$wealth)
mental2$health <-as.integer(mental2$health)
#.변수 빈도 확인
table(mental$suicide)
table(mental$health)
table(mental$wealth)
table(mental$satisfaction)

#.변수 정리
mental$satisfaction<-mental$satisfaction-1
mental$wealth<-mental$wealth-1

table(mental$wealth)
table(mental$satisfaction)

mental2$satisfaction<-mental2$satisfaction-1
mental2$wealth<-mental2$wealth-1

table(mental2$wealth)
table(mental2$satisfaction)

#.범주형변수를 문자형으로 변경:오류방지
mental$age<-as.character(mental$age)
mental$sex<-as.character(mental$sex)
mental$area<-as.character(mental$area)

table(mental$sex)
table(mental$age)
table(mental$area)

mental$age<-ifelse(mental$age=="19~29세","20대",mental$age)
table(mental$age)

mental2$age<-as.character(mental2$age)
mental2$sex<-as.character(mental2$sex)
mental2$area<-as.character(mental2$area)

table(mental2$sex)
table(mental2$age)
table(mental2$area)

mental2$age<-ifelse(mental2$age=="19~29세","20대",mental2$age)
table(mental2$age)

#.결측치와 이상치 확인
summary(mental)
summary(mental2)

#.데이터 분석 1.빈도분석
#.성별빈도분석
mental%>%
  group_by(sex)%>%
  summarise(n=n())%>% #sex변수의 범주별 빈도 계산
  mutate(total=sum(n), #sex변수의 빈도 총계
         pct=round(n/total*100,1)) #sex변수의 범주별 비율

  mental2%>%
    group_by(sex)%>%
    summarise(n=n())%>% #sex변수의 범주별 빈도 계산
    mutate(total=sum(n), #sex변수의 빈도 총계
           pct=round(n/total*100,1)) #sex변수의 범주별 비율

#.연령대별빈도분석
mental%>%
  group_by(age)%>%
  summarise(n=n())%>% #age변수의 범주별 빈도 계산
  mutate(total=sum(n), #age변수의 빈도 총계
         pct=round(n/total*100,1)) #age변수의 범주별 비율

mental2%>%
  group_by(age)%>%
  summarise(n=n())%>% #age변수의 범주별 빈도 계산
  mutate(total=sum(n), #age변수의 빈도 총계
         pct=round(n/total*100,1)) #age변수의 범주별 비율

##.연령별 남녀 집단 평균 만족도(추가 조사)

life_satisfaction_analysis <- mental %>%
  group_by(age, sex) %>%
  summarise(avg_satisfaction = mean(satisfaction)) %>%
  arrange(desc(avg_satisfaction))  

top_groups <- life_satisfaction_analysis %>%
  top_n(4, wt = avg_satisfaction) 
##
life_satisfaction_analysis <- mental2 %>%
  group_by(age, sex) %>%
  summarise(avg_satisfaction = mean(satisfaction)) %>%
  arrange(desc(avg_satisfaction))  

top_groups2 <- life_satisfaction_analysis %>%
  top_n(4, wt = avg_satisfaction) 



#.교차분석
table(mental$sex, mental$age)
round(prop.table(table(mental$sex, mental$age),1)*100,1) #교차백분율계산
chisq.test(mental$sex, mental$age)

table(mental2$sex, mental2$age)
round(prop.table(table(mental2$sex, mental2$age),1)*100,1) #교차백분율계산
chisq.test(mental2$sex, mental2$age)

#.평균분석
mental%>%
  summarise(m1=mean(suicide),m2=mean(satisfaction),
            m3=mean(loneliness),m4=mean(family_belief),
            m5=mean(wealth),m6=mean(health))

mental2%>%
  summarise(m1=mean(suicide),m2=mean(satisfaction),
            m3=mean(loneliness),m4=mean(family_belief),
            m5=mean(wealth),m6=mean(health))

#.회귀분석:삶의 만족도와 외로움이 자살충동에 미치는 영향


install.packages("car")
library(car)

RA <-lm(data=mental, suicide~satisfaction+loneliness)
summary(RA)
vif(RA)

RA2 <-lm(data=mental2, suicide~satisfaction+loneliness)
summary(RA2)
vif(RA2)

#.상관분석:삶의 만족도와 외로움의 상관관계
cor.test(mental$satisfaction,mental$loneliness)
cor.test(mental2$satisfaction,mental2$loneliness)


#.회귀분석:가족신뢰도, 경제안정도, 건강상태가 삶의 만족도에 미치는 영향
install.packages("ztable")
library(ztable)

RA <-lm(data=mental, satisfaction~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)

RA <-lm(data=mental2, satisfaction~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)

#.회귀분석:가족신뢰도, 경제안정도, 건강상태가 외로움에 미치는 영향
RA <-lm(data=mental, loneliness~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)

RA <-lm(data=mental2, loneliness~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)

##.회귀분석:가족신뢰도, 경제안정도, 건강상태가 자살충동에 미치는 영향(추가)
RA <-lm(data=mental, suicide~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)

RA <-lm(data=mental2, suicide~family_belief+wealth+health)
summary(RA)
options(ztable.type="viewer")
ztable(RA)
vif(RA)


#.독립표본t검정:성별 삶의 만족도 차이
t.test(data=mental, satisfaction~sex)
t.test(data=mental2, satisfaction~sex)


#.연령대별 삶의 만족도 차이
mental%>%
  group_by(age)%>%
  summarise(m=mean(satisfaction))%>%
  arrange(desc(m))

mental2%>%
  group_by(age)%>%
  summarise(m=mean(satisfaction))%>%
  arrange(desc(m))

#.지역별 삶의 만족도 분석과 그래프 작성
area_satisfaction <-mental%>%
  group_by(area) %>%
  summarise(m=mean(satisfaction)) %>%
  arrange(desc(m))

ggplot(data=area_satisfaction, aes(x=reorder(area,m),y=m))+geom_col()+ggtitle("지역별 만족도")+xlab("지역")+ylab("만족도")+coord_flip()

area_satisfaction2 <-mental2%>%
  group_by(area) %>%
  summarise(m=mean(satisfaction)) %>%
  arrange(desc(m))

ggplot(data=area_satisfaction2, aes(x=reorder(area,m),y=m))+
  geom_col()+
  ggtitle("지역별 만족도")+
  xlab("지역")+
  ylab("만족도")+
  coord_flip()

